...
class GUI:
    ...
    def __init__(self):
        self.core = Core()
        self.state_grid =  self.core.get_state_grid()
    ...
    def load_map(self):
            ...
            for i in range(self.tablesize):
                for j in range(self.tablesize):
                    if self.state_grid[i][j] not in Tile.PLAYERS:
                    ...
                    self.map.create_rectangle(
                            i * self.cellsize, j * self.cellsize,
                            (i + 1) * self.cellsize,
                            (j + 1) * self.cellsize, fill=self.colour_list[self.state_grid[i][j]])
    ...
    def drop_token(self, x, y):
        move_dir = self.core.get_dir(x, y)

        # recursively drops the token
        if (move_dir) == (0, 0):
            if self.core.insert_token(x, y) != -1:
                self.game_end()
            self.state_grid = self.core.get_state_grid()
            self.load_map()
        ...
        self.root.after(250, self.drop_token, x, y)
